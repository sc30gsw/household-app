// ドロップダウンのサブメニューを表示する処理(詳細画面用)
function subMenuSec() {
	// ドロップダウンのメインカテゴリー要素(li要素)を取得
	const parent = document.querySelectorAll('.dropdown-submenu-sec');
	const item = Array.prototype.slice.call(parent, 0);

	item.forEach(function(element) {
		// メインカテゴリー要素のリンクを取得する
		const lists = document.querySelectorAll('.c_name_sec');
		const list = Array.prototype.slice.call(lists, 0);

		list.forEach(function(l) {
			// メインカテゴリー要素(li要素)にマウスが乗った時クラスを追加する
			element.addEventListener("mouseover", function() {
				element.querySelector(".sub_menu_sec").classList.add("open-sec");
			}, false);
			// メインカテゴリー要素(li要素)からマウスが離れた時クラスを除外する
			l.addEventListener("mouseout", function() {
				element.querySelector(".sub_menu_sec").classList.remove("open-sec");
			}, false);
		});
